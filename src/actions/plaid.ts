...
async function initialHistoricalSync(
  ...
  // Plaid needs time to prepare transaction data after initial connection
  // Retry with exponential backoff: 2s, 4s, 8s, 16s (up to 30s total wait)
  const maxRetries = 5;
  let lastError: unknown = null;

  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      // Fetch first page to check if transactions are ready
      ...
      lastError = error;
      console.log(`Attempt ${attempt + 1}/${maxRetries} failed, waiting before retry...`);
